...
Execute as e.g.: PYTHONPATH=../framework python3 ex5var1_msrtf_machine.py
"""
...
_DEBUG = True
SIZE_A, CL_K = 5, 5
TAG = 'ex5var1-msrtf-machine'
DATA_FILENAME = 'ex5var1_msrtf.npz'
...
ode_ts = numpy.linspace(0, 80.0, 1001)
...
if True:
  ...
  ax.set_xlabel('time')
  ax.legend(loc='right')
  prob_seq4 = ode_ys[-1].reshape([5]*5).sum(axis=-1)
  n_relevant = [numpy.argmin(prob_seq4.ravel()),
                numpy.argmax(prob_seq4.ravel())]
  for n in n_relevant:
    print([[(n // 5**k) % 5 for k in reversed(range(4))] for n in [0, 241]],
          prob_seq4.ravel()[n])
  fig.savefig('ex5var1_msrtf.pdf')
  fig.show()
